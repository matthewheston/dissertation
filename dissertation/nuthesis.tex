...
There are likely mechanistic differences in how responsiveness is interpreted across a wide breadth of scenarios. Participants in the experiment by \citet{mclaughlin1982awkward}, for example, interpreted responsiveness delays on a scale of a few seconds or less, whereas participants in the experiment by \citet{kalman2011online} were discussing email delays up to  several days. Despite these different time scales, however, the meaning of how responsiveness is interpreted seems relatively stable across different contexts. Interview participants in \citet{church2013s} discussed responsiveness in terms of immediacy and paying attention in a conversation. Similarly, experimental work on FtF responsiveness found it related to perceptions of conversational involvement and~\citep{newman1982sounds}. This is to say that although the interpretation process of responsiveness will differ across different FtF and CMC contexts, since the meaning of responsiveness is relatively stable, findings from FtF conversations may still be useful to inform out understanding of responsiveness in CMC.
...
Availability was captured using a single measure at one point in time. However, there is likely a temporal effect on availability that this does not capture. Imagine, for example, a person in a movie theater. Given that it is impolite to use your phone during the movie, the person may rate themselves as unavailable throughout the entirety of the movie. However, if they receive a message at the beginning of the movie, they will continue to be unavailable for 90 minutes. If they receive a message in the last 5 minutes of the movie, their availability will soon change and they may be able to respond to the message then.

To test for the presence of such temporal effects, we can compare SIA messages received by the same participant within some time window and examine differences in availability and responsiveness. However, since SIA messages occur fairly infrequently, an analysis of SIA messages which occurred close in time to another SIA message results in a fairly small dataset.

Examining SIA messages that were received within an hour of another SIA message results in 205 messages. The distribution of the difference in availability between messages received within an hour of one another follows a normal distribution with mean 0 and a standard deviation of 1.5. In other words, if an SIA message was received within an hour of another SIA message, there tended not to be a difference in reported availability for each message. In examining differences in response time in such messages, in 90\% of cases, response times were within a 15 minute difference to one another. For example, imagine a participant who received an SIA message and it took them 20 minutes to respond. If they were to receive another SIA within one hour, this data suggests in 90\% of cases they would respond to this SIA in between 5 and 35 minutes.

Although this analysis is based on a small subset of data, in the available cases, availability and responsiveness tended to stay fairly stable over an hour long period.
...
A final effect of the decision to prompt users at the moment of response is that retrospective self-reporting happened at varying timescales, which could affect a participant's ability to correctly assess themselves. For example, a participant who responded to an SIA will have a much better idea about their availability at the time a message was received than a participant who responded 3 days later, who will likely rely much more on heuristics to retroactively assess their availability. Although participants were shown a timestamped copy of the message they received to assist them in their assessment, it may be the case that longer delayed responses are less accurate. While the benefits of the chosen experience sampling schedule were considered to outweigh this issue, future work may use different experience sampling methods to collect self-reporting in a more consistent timeframe.
...
It is also possible that the sample for this study, which consisted in part of individuals who make an effort to browse Craigslist and Reddit for studies, differs in some way from the general population, limiting the generalizability of the findings. It may be the case, for example, that such individuals have more free time to find such studies and therefore have different availability scheduled. It is difficult to assess if this is the case with the data collected. However, general texting habits such as amount and frequency of texting of the study sample do match texting behaviors found in other studies~\citep[e.g.,][]{smith2015us}, so the sample does seem similar to the broader texting population in that respect. While an observational study such as this one can not guarantee a lack of sample bias, future experimental or quasi-experimental studies may be useful for mitigating these effects through random assignment.
...
\begin{table}[!htbp] \fontsize{6.5}{6.8}\selectfont \centering 
\begin{tabular}{@{\extracolsep{5pt}}lcccc} 
\\[-1.8ex]\hline 
\hline \\[-1.8ex] 
 & \multicolumn{4}{c}{\textit{Dependent variable:}} \\ 
...
\begin{enumerate}
	\item What is your email address? [This is required to send you instructions on how to install the app and to send you your Amazon gift code upon study completion.]
	\item What year were you born?
	\item What is your gender?
		\begin{itemize}
		\item Male
		\item Female	
		\end{itemize}
	\item What is your ethnicity? [Check all that apply]
		\begin{itemize}
			\item Asian
		...
		\end{itemize}
	\item Who pays your smartphone bill?
		\begin{itemize}
		\item You
		\item Someone else	
		\end{itemize}
	\item Are you aware of any limits on your texting plan?
		\begin{itemize}
		\item No
		\item Yes (explain)
		\end{itemize}
	\item Are you aware of any limits on your data plan?
		\begin{itemize}
		\item No
		\item Yes (explain)
		\end{itemize}
	\item What timezone do you live in?
		\begin{itemize}
			\item Hawaii–Aleutian Time Zone
			\item Alaska Time Zone
			\item Pacific Time Zone
			\item Mountain Time Zone
			\item Central Time Zone
			\item Eastern Time Zone
		\end{itemize}
	\item Around what time do you normally go to bed?
	\item Around what time do you usually wake up?
	\item Generally describe your work schedule. (E.g., Do you work nights? Do you work weekends?)
	\item What other messaging apps do you use on your phone? (e.g., WhatsApp, Facebook Messenger, Google Hangouts, etc.)
	\item Compared to these other messaging apps, how often would you say you use SMS text messaging?
		\begin{itemize}
			\item A lot less often
			\item Somewhat less often
			\item About the same amount
			\item More often
		\end{itemize}
...
\section{Post-Study Relational Variables Scales}

\subsection{Intimacy Scale}

\begin{itemize}
\item How often do you feel close to him/her? 1(Very Rarely) – 5 (Always)
\item How close do you feel to him/her most of the time? 1(Not much) – 5 (Very much)
\item How important is your relationship with him/her? 1(Not much) – 5 (Very much)	
\end{itemize}

\subsection{Power Scale}
How much do you agree with the following on a scale of 1-5?

\begin{itemize}
\item He/she can influence my decision making.
\item He/she has power over what I do.
\item He/she tends to structure and lead our conversations.
\end{itemize}
...
\begin{itemize}
\item We live together.
\item In the same city, but we don't live together.
\item In the same state, but not the same city.
\item In the same timezone, but not the same state.
\item In a different timezone.	
\end{itemize}
...
\begin{figure}[h]
\centering
\includegraphics[width=.7\textwidth]{figures/app_init}
\caption{Screenshot of app initialization screen.}
\label{fig:app_init}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=.7\textwidth]{figures/app_screen}
\caption{Screenshot of app questionnaire screen.}
